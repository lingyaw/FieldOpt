...
TEST_F(MathTest, complete_model) {
    ...
    for (int j = 0; j < 6; ++j) {
        std::cout << "model = " << model_approx.evaluate(test_model.get_points().at(j)) << " same as "
                  << test_model.silly_function(test_model.get_points().at(j)) << std::endl;
        EXPECT_NEAR(model_approx.evaluate(test_model.get_points().at(j)),
                    test_model.silly_function(test_model.get_points().at(j)),
                    10e-6);
    }
}
